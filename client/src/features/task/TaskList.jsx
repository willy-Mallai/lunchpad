...
function TaskList({ task, isEditingId, setIsEditingId }) {
  ...
  const isEdit = task._id === isEditingId;

  async function handleSave(taskId) {
    if (!editTask.trim()) return;
    try {
      const updateData = await updateTask(taskId, { title: editTask });
      dispatch({ type: "editTask", payload: updateData });
    } catch (err) {
  ...
  async function toggleCmplete(taskId) {
    try {
      const updateData = await updateTask(taskId, {
    ...
    >
      <div className="flex items-center gap-3 flex-1 min-w-0">
        <input
          ...
          onChange={() => toggleCmplete(task._id)}
