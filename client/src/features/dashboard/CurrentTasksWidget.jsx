...
import { updateTask } from "../task/taskService";

function CurrentTasksWidget() {
  const { state: tasksState, dispatch: taskDispatch } = useTask();
    ...
    <div className="p-6 rounded-2xl bg-white border border-[#2E365A]/15 backdrop-blur-xl shadow-sm space-y-5">
      ...
      {tasks.length === 0 ? (
        <div className="text-center py-8 border-2 border-dashed border-[#2E365A]/20 rounded-xl">
          ...
          <p className="text-sm font-medium text-[#6B597F]">
            No tasks added yet
          </p>
          <Link
            to="/task"
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                <button
                  type="button"
                  onClick={async () => {
                    const updateData = await updateTask(task._id, {
                      completed: !task.completed,
                    });

                    taskDispatch({ type: "editTask", payload: updateData });
                  }}
